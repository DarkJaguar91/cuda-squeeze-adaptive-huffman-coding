...
class Node {
public:
	Node(float * d = NULL, Node * par = NULL, int w = 0, int c = 0) :
			data(d), weight(w), code(c), parent(par), left(NULL), right(NULL) {
	}
...
public:
	...
	std::vector<byte> & encodeValue(float data) {
		...
		if (root == NULL) { // root null
			root = new Node();
			root->right = new Node(new float(data), root, 1, 1);
...
//			checkAndAdd(root->right);
		...
		if (uniqueList.count(data) > 0) { // exists
			...
			vector<byte> code = getCode(n);

			update(n);
			return code;
		} else { // dnts exist
			nullPar->left = new Node(NULL, nullPar, 0, nullPar->code - 2);
			nullPar->left->right = new Node(new float(data), nullPar->left, 1,
					nullPar->left->code - 1);

			vector<byte> code = getCode(nullPar->left->right);
...
//		checkAndAdd(node);
	...
	void exchangeNodes(Node * n1, Node * n2) {
		...
		std::swap(n1->code, n2->code); // swap the parent pointerss
