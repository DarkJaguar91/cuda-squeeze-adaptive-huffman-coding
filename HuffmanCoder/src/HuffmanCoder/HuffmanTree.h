...
public:
	...
	HuffmanTree(frequencyMap & map, const HuffCode & code,
...
private:
	...
	void setUpTree(std::vector<float> & values,
		...
		splitCodes(code, values);
	}

	void splitCodes(const HuffCode & code, std::vector<float> & values) {
		longValue cpos = 0;
		while (cpos < code.size() - 8) {
			HuffCode c;

			if (code[cpos] == false && code[cpos + 1] == false) {
				std::copy(code.begin() + cpos, code.begin() + cpos + 10,
						std::back_inserter(c));
			} else if (code[cpos] == false && code[cpos + 1] == true) {
				std::copy(code.begin() + cpos, code.begin() + cpos + 18,
						std::back_inserter(c));
			} else if (code[cpos] == true && code[cpos + 1] == false) {
				std::copy(code.begin() + cpos, code.begin() + cpos + 26,
						std::back_inserter(c));
			} else {
				std::copy(code.begin() + cpos, code.begin() + cpos + 34,
						std::back_inserter(c));
			}

			cpos += c.size();

			values.push_back(*rmap[c]);
		}
	}
