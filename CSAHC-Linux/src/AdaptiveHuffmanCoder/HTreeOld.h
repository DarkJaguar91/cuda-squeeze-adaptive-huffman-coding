...
struct NodeOld {
	 ...
	 * par the parent node
	 */
	NodeOld(T * d, NodeOld * par = NULL, long w = 0, long h = 0) :
	...
	long height;
	NodeOld<T> * parent; // the parent node
	NodeOld<T> * left; // left child node
	NodeOld<T> * right; // right child node
};
...
public:
	 ...
	 */
	void printFromNode(NodeOld<T> * n, long depth) {
		using namespace std;
...
//			cout << "(NULL)\n";
	...
	std::vector<T> decode(std::vector<byte> & values, long numValues) {
		...
		NodeOld<T> * tmp = root; // set a temp node to root

		vector<T> output; // the output list of numbers initialiser

		// if root is null
		if (tmp == NULL) {
			root = new NodeOld<T>(NULL, NULL, 0, 0); // create the root node
			root->right = new NodeOld<T>(new T(uniqueValueOrderedList.at(0)), root, root->height + 1); // create the first data node with the first found value
		...
		while (values.size() > 0) { // while we still have values to read
			if (long(output.size()) == numValues)
				return output;
			NodeOld<T> * lookAt = NULL; // create a temp child node
			...
			if (lookAt == NULL){ // value not found.. add it
				tmp->left = new NodeOld<T>(NULL, tmp); // add the child that is missing
				tmp->left->right = new NodeOld<T>(new T(uniqueValueOrderedList.at(0)), tmp->left, 1); // get the value from the list
	...
	std::vector<byte> encode(T data) {
		...
		if (root == NULL) { // if the tree is new (root is null)
			root = new NodeOld<T>(NULL); // set the root node
			root->right = new NodeOld<T>(new T(data), root, 1, 1); // set the first value
		...
		NodeOld<T> * n = findNode(root, data); // look for the value in our tree
		...
		if (n == NULL) { // element not found in tree
			...
			n->left = new NodeOld<T>(NULL, n, 0, n->height + 1); // create the new joiner
			n->left->right = new NodeOld<T>(new T(data), n->left, 1, n->height + 2); // add the data leaf node
	 ...
	 */
	std::vector<byte> getCode(NodeOld<T> * n) {
		using namespace std;

		vector<byte> out; // the code temp value

		NodeOld<T> * par = n->parent; // get the parent of the node
	 ...
	 */
	void update(NodeOld<T> * n) {
		using namespace std;

		NodeOld<T> * tmp = getMostSig(root, n->weight);
...
private:
	NodeOld<T> * root; // the root nodes pointer
	NodeOld<T> * nullParent;
//	std::vector< std::vector<Node<T> * > > blockList;
	 ...
	 */
	void exchangeNodes(NodeOld<T> * n1, NodeOld<T> * n2) {
	 ...
	 */
	NodeOld<T> * getMostSig(NodeOld<T> * n, long weight) {
		using namespace std;

		vector<NodeOld<T> *> que; // que for breadth tree searching
	 ...
	 */
	NodeOld<T> * findNULLParent(NodeOld<T> * n) {
		if (n->right == NULL) { // if right is null
			...
			NodeOld<T> * save = findNULLParent(n->left); // recursively check left sub tree
	 ...
	 */
	NodeOld<T> * findNode(NodeOld<T> * n, T data) {
		if (n == NULL) { // if null node
			return NULL; // wrong node return null
		} else if (n->data == NULL) { //if not a leaf node
			NodeOld<T> * save = findNode(n->right, data); // check right sub tree (right first cus leaves are more likely on the right)
